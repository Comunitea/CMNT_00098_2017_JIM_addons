...
class SaleOrderLine(models.Model):
    ...
    def _template_discontinued(self):
        ...
        if hasattr(self, "product_id") and hasattr(
            self.product_id, "discontinued_product"
        ):
    ...
    def ts_product_id_change(
        ...
        result = super(SaleOrderLine, self).ts_product_id_change(
            product_id, partner_id, pricelist_id
        )

        result.update({"discontinued": product.discontinued_product})
