...
class ProductTemplate(models.Model):
    ...
    discontinued_product = fields.Boolean(
        "Discontinued",
        default=False,
        help="If checked, the product will not be sold in main company",
    )
    product_size_width = fields.Float(
        "Width",
        compute="_compute_size",
        inverse="_set_size",
        store=True,
        help="Product max width in cm",
    )
    product_size_height = fields.Float(
        "Height",
        compute="_compute_size",
        inverse="_set_size",
        store=True,
        help="Product max height in cm",
    )
    product_size_depth = fields.Float(
        "Depth",
        compute="_compute_size",
        inverse="_set_size",
        store=True,
        help="Product max depth in cm",
    )
    volume = fields.Float(
        compute="_compute_size",
        inverse=False,
        digits=(3, 6),
        store=False,
        help="Computed volume of the product (cube formula) in m³",
    )
    ...
    @api.depends(
        "product_variant_ids",
        "product_variant_ids.product_size_width",
        "product_variant_ids.product_size_height",
        "product_variant_ids.product_size_depth",
    )
    def _compute_size(self):
        ...
        for template in unique_variants:
            template.product_size_width = (
                template.product_variant_ids.product_size_width
            )
            template.product_size_height = (
                template.product_variant_ids.product_size_height
            )
            template.product_size_depth = (
                template.product_variant_ids.product_size_depth
            )
        ...
        if "discontinued_product" in values:
            for record in self:
                for variant in record.product_variant_ids:
                    variant.discontinued_product = values[
    ...
    def ts_get_grid_structure(self, template_id, partner_id, pricelist_id):
        ...
        result = super(ProductTemplate, self).ts_get_grid_structure(
            template_id, partner_id, pricelist_id
        )
                        ...
                        result["str_table"][x][y].update(
                            {
                                ...
                                and product.discontinued_product
                                or False
                            }
                        )
...
class ProductProduct(models.Model):
    ...
    discontinued_product = fields.Boolean(
        "Discontinued",
        default=False,
        help="If checked, the variant will not be sold in main company",
    )
    product_size_width = fields.Float("Width", help="Product max width in cm")
    product_size_height = fields.Float(
        "Height", help="Product max height in cm"
    )
    ...
    @api.depends(
        "product_size_depth", "product_size_width", "product_size_height"
    )
    def _compute_volume(self):
        for record in self:
            volumeInCm = calcCubeVolume(
                record.product_size_width,
                record.product_size_height,
                record.product_size_depth,
            )
