...
class ProductProduct(models.Model):
    ...
    def _search_global_real_stock(self, operator, value):
        if value == 0.0 and operator in ("=", ">=", "<="):
            return self._search_global_product_quantity(
                operator, value, "global_real_stock"
            )

    def _search_global_avail_stock(self, operator, value):
        return self._search_global_product_quantity(
            operator, value, "global_available_stock"
        )

    def _search_web_global_stock(self, operator, value):
        return self._search_global_product_quantity(
            operator, value, "web_global_stock"
        )
    ...
    def _calculate_globals(self):
        not_deposit_ids = (
            ...
            .sudo()
            .search([("deposit", "!=", True), ("usage", "=", "internal")])
            .ids
        )
        company_ids = (
        ...
        self._cr.execute(
            ...
            [tuple(self.ids), tuple(company_ids)],
        )
        ...
        moves_v = self.env["stock.move"].read_group(
            moves_v_domain, ["product_id", "product_qty"], ["product_id"]
        )
        dict_v = dict(
            [(x["product_id"][0], x["product_qty"]) for x in moves_v]
        )
        ...
        a = dict(
            [
                (p, global_real_stock[p] - company_real_stock[p])
                for p in self.ids
                if p in global_available_stock.keys()
            ]
        )
        b = dict(
            [
                (
                    p,
                    global_available_stock[p]
                    - sale_line_data.get(p, 0)
                    - company_available_stock[p],
                )
                for p in self.ids
                if p in global_available_stock.keys()
            ]
        ...
        return dict(
            [
                (
                    p,
                    {
                        "global_real_stock": a[p],
                        "global_available_stock": b[p],
                    },
                )
                for p in self.ids
                if p in global_available_stock.keys()
    ...
    def _compute_global_stock(self):
        ...
        for product in self.filtered(lambda x: x.id in res.keys()):
            ...
            product.global_available_stock = res[product.id][
                "global_available_stock"
            ]
            ...
            if product.bom_count:
                ...
                for bom in boms:
                    ...
                    for line in bom.bom_line_ids:
                        if line.product_id.id in res:
                            global_available_stock = res[line.product_id.id][
                                "global_available_stock"
                            ]
                        else:
                            global_available_stock = (
                ...
                for line in bom_lines:
                    if line.product_qty:
                        variants = (
                            line.bom_id.product_id
                            or line.bom_id.product_tmpl_id.product_variant_ids
                        )
