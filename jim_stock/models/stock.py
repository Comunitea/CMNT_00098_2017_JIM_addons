...
class StockPicking(models.Model):
    ...
    def _get_label_data(self):
        ...
        res["cliente_atencion"] = (
            self.partner_id.default_contact_person
            and unidecode(self.partner_id.default_contact_person)
            or unidecode(self.partner_id.name)
        )
        if self.neutral_document:
            res["nombre_remitente"] = unidecode(
                self.sale_id.partner_id.name or ""
            )
            res["direccion_remitente"] = unidecode(
                self.sale_id.partner_id.street or ""
            )
            res["codPostal_remitente"] = unidecode(
                self.sale_id.partner_id.zip or ""
            )
            res["poblacion_remitente"] = unidecode(
                self.sale_id.partner_id.city or ""
            )
...
class StockQuantPackage(models.Model):
    ...
    packaging_id_code = fields.Char(
        related="packaging_id.shipper_package_code"
    )
...
class StockQuant(models.Model):
    ...
    def create(self, vals):
        ...
        if self._context.get("create_date", False):
            self.env.cr.execute(
                "update stock_quant set create_date='%s' where id=%s"
                % (self._context["create_date"], res.id)
            )
        return res
...
class StockMove(models.Model):
    ...
    def create(self, vals):
        ...
        if self._context.get("create_date", False):
            self.env.cr.execute(
                "update stock_move set create_date='%s' where id=%s"
                % (self._context["create_date"], res.id)
            )
        return res
