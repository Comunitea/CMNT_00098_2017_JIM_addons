...
class PurchaseOrderLine(models.Model):
    ...
    purchase_order_line_ids = fields.One2many(
        "purchase.order.line", compute="get_same_product_purchase_line_ids"
    )
    ...
    def _compute_qty_received(self):
        ...
        for line in self:
            ...
            if line.product_id.type not in ["consu", "product"]:
                ...
                continue
            total = 0.0
            returns = line.move_ids.filtered(
                lambda x: x.origin_returned_move_id
            )
...
class PurchaseOrder(models.Model):
    ...
    def _default_picking_type(self):
        ...
        domain_wh = [
            ("company_id", "=", self.env.user.company_id.partner_id.id)
        ]
...
class AccountInvoice(models.Model):
    ...
    def action_add_purchase_invoice_wzd(self):
        ...
        res_id = wizard_obj.create(
            {"partner_id": self.partner_id.id, "account_invoice_id": self.id}
        )
        return {
