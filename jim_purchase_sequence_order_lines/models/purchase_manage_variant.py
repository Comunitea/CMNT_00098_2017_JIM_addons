...
class PurchaseManageVariant(models.TransientModel):
    ...
    def _onchange_product_tmpl_id(self):
        ...
        self.template_sequence = max(purchase_order.order_line.mapped('template_sequence'))+1
        return super(PurchaseManageVariant, self)._onchange_product_tmpl_id()
    ...
    def button_transfer_to_order(self):
        ...
        if context['active_model'] == 'purchase.order.line':
            ...
            purchase_order = record
        for line in purchase_order.order_line.filtered(lambda x: x.product_id.product_tmpl_id == self.product_tmpl_id):
            sequence = line.product_id.get_variant_sequence(sequence_origin=self.template_sequence)
            vals = {
                'sequence': sequence,
