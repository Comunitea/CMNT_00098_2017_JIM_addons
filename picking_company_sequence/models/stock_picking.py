...
class StockPickingType(models.Model):
    ...
    property_sequence_id = fields.Many2one(
        "ir.sequence", "Reference Sequence (Company)", company_dependent=True
    )
...
class StockPicking(models.Model):
    ...
    def create(self, vals):
        ...
        if vals.get("picking_type_id", False) and vals.get(
            "company_id", False
        ):
            picking_type_id = (
            ...
            property_sequence = (
                picking_type_id and picking_type_id.property_sequence_id
            )
