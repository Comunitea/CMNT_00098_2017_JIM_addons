...
class purchase_order(models.Model):
    ...
    def _prepare_sale_order_data(
        ...
        """
        partner_addr = partner.sudo().address_get(
    ...
    def _prepare_sale_order_line_data(self, line, company, sale_id):
        vals = super(purchase_order, self)._prepare_sale_order_line_data(
        ...
        if line.procurement_ids and line.procurement_ids[0].route_ids:
            ...
            vals["move_dest_IC_id"] = (
                line.procurement_ids[0].move_dest_id.id or False
            )
            vals["purchase_line_IC"] = line.id
        vals["name"] = (
            line.product_id and line.product_id.display_name or line.name
        )
        return vals
