...
class PickingType(models.Model):
    ...
    count_picking_review = fields.Integer(
...
class StockPicking(models.Model):
        ...
        sales = self.env["sale.order"].search(
            [
                (
                    ...
                    "in",
                    [ic_purchase.mapped("group_id").id],
                )
            ]
        )
    ...
    orig_sale_id = fields.Many2one(
        "sale.order", "Origen", compute=_compute_orig_sale, multi=True
    )
    orig_sale_str = fields.Char(
        "Sale order", compute=_compute_orig_sale, multi=True
    )
    ready = fields.Boolean("Revision Ready", default=False, readonly=True)
    unreserve_done = fields.Boolean(
        "Unreserve done", default=False, copy=False
    )

    @api.model
    def _prepare_values_extra_move(self, op, product, remaining_qty):
        vals = super(StockPicking, self)._prepare_values_extra_move(
    ...
    def action_cancel(self):
        for picking in self:
            if picking.picking_type_id.code == "outgoing":
                ic_purchases = self.env["purchase.order"].search(
                    [
                ...
                if ic_purchases:
                    for purchase in ic_purchases:
                        picking_in_ids = purchase.picking_ids.filtered(
            ...
            if picking.purchase_id.intercompany:
                ...
                for sale in ic_sale:
                    sale_pickings = sale.picking_ids.filtered(
    ...
    def propagate_pick_values(self):
        ...
        try:
            ...
            if next_pick:
                _logger.info(
                    "Propago valores de %s al albaran %s"
                    % (pick_id.name, next_pick.name)
                )
                ...
                if pick_id.carrier_id and not next_pick.carrier_id:
                    _logger.info(
                        "Transportista:{}".format(pick_id.carrier_id.id)
                    )
                ...
                _logger.info(
                    "No se ha encontrado pick al que propagar los datos"
                )
        ...
        if self.picking_type_id.code == "outgoing":
            ...
            if ic_purchases:
                ...
                for ic_purchase_picking in picking_in_ids:
                    ...
                    ops_to_do = ic_purchase_picking.pack_operation_product_ids.filtered(
                        lambda x: x.qty_done > 0
                    )
                    if ops_to_do:
                        ic_purchase_picking.do_transfer()

        message = "El usuario <em>{}</em> ha validado el albarán {}<ul><li>Último estado: {}</li><li>Hora de validacion: {}</li>".format(
            self._context.get("user_name", user_id.name),
            self.name,
            self.state,
            fields.Datetime.now(),
        )
    ...
    def propagate_op_qty(self, picking):
        for op in self.pack_operation_product_ids:
            pick_op = picking.pack_operation_product_ids.filtered(
                lambda x: x.product_id.id == op.product_id.id
            )
...
class StockMove(models.Model):
    ...
    move_purchase_IC_id = fields.Many2one(
        "stock.move",
        "Purchase Move IC",
        copy=False,
        index=True,
    ...
    def _prepare_procurement_from_move(self):
        ...
        vals["move_dest_IC_id"] = (
            self.move_dest_IC_id.id or self.move_dest_id.move_dest_IC_id.id
        )
        if (
            self.procurement_id.sale_line_id.order_id.partner_id.id
    ...
    def propagate_new_moves(self):
        ...
        for move in self:
            ...
            if (
                product_move.move_dest_id
                and not product_move.picking_id.purchase_id.intercompany
            ):
                picking = product_move.move_dest_id.picking_id
                vals = move.prepare_propagate_vals(
                    product_move.move_dest_id, picking
                )
    ...
    def propagate_assign_IC(self):
        ...
        pick_purchase_IC = moves_done.mapped(
        ...
        if pick_dest_IC and previous_IC_sale:
            ...
            mov_to_force |= moves_done.filtered(
                lambda move: move.move_dest_id.move_purchase_IC_id
                and move.move_dest_id.move_purchase_IC_id.state
                in ("waiting", "confirmed")
            ).mapped("move_dest_id.move_purchase_IC_id")
            mov_to_force |= moves_done.filtered(
                lambda move: move.move_dest_IC_id.state
                in ("waiting", "confirmed")
            ).mapped("move_dest_IC_id")
            if mov_to_force:
    ...
    def propagate_assign_IC_old(self):
        ...
        if pick_dest_IC and previous_IC_sale:
            ...
            pick_purchase_IC = self.filtered(
                lambda x: x.state == "done"
            ).mapped("move_dest_id.move_purchase_IC_id.picking_id")
            if pick_purchase_IC and pick_purchase_IC.state != "done":
            ...
            for move in self.sudo():
                if pick_dest_IC:
                    if (
                        move.state == "done"
                        and move.move_dest_IC_id
                        and move.move_dest_IC_id.state
                        in ("waiting", "confirmed")
                    ):
                ...
                if pick_dest_IC:
                    if (
                        move.state == "done"
                        and move.move_dest_id.move_purchase_IC_id
                        and move.move_dest_id.move_purchase_IC_id.state
                        in ("waiting", "confirmed")
                    ):
    ...
    def split(self, qty, restrict_lot_id=False, restrict_partner_id=False):
        ...
        new_move = super(StockMove, self).split(
            qty, restrict_lot_id, restrict_partner_id
        )
        ...
        if new_move and self.sudo().move_purchase_IC_id:
            new_move_prop_purchase_IC = self.sudo().move_purchase_IC_id.split(
                qty
            )
            self.env["stock.move"].browse(new_move).sudo().write(
        ...
        if new_move and previous_IC_sale and self.sudo().move_dest_IC_id:
            self.env["stock.move"].browse(new_move).sudo().write(
                {
                    "move_dest_IC_id": self.env["stock.move"]
                    .browse(new_move)
                    .move_dest_id.move_purchase_IC_id.move_dest_id.id
                }
            )
