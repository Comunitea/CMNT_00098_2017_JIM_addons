...
class ProductTemplate(models.Model):
    ...
    intercompany_price = fields.Float(
        "Intercompany Price", digits=dp.get_precision("Product Price")
    )
...
class ProductProduc(models.Model):
    ...
    def get_intercompany_price(self, company_id, partner_id):
        ...
        if not float_is_zero(self.intercompany_price, precision_digits=2):
            return self.intercompany_price
        # Compra realizada desde Jim o SE a EME
        if company_id in (1, 8, 16) and partner_id == 11:
            pricelist_id = 39  # tarifa de produccion
            pricelist = self.env["product.pricelist"].browse(pricelist_id)
            price = pricelist.get_product_price(
                self, 1.0, self.env["res.partner"].browse(partner_id)
            )
            return price
