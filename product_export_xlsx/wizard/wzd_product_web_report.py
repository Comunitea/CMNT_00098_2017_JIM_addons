...
class WizardValuationHistory(models.TransientModel):
    ...
    web_visible = fields.Selection(
        [("yes", "Visible"), ("no", "No visible"), ("all", "Todos")],
        string="Visible en web",
        default="all",
    )
    stock_available = fields.Selection(
        [
            ("yes", "Disponible"),
            ("no", "No disponible"),
            ("all", "Todos"),
            ("all_inactive", "Todos, incluido no activos"),
        ],
    ...
    stock_field = fields.Selection(
        (
            ...
            ("web_global_stock", "Web global stock"),
        ),
        default="web_global_stock",
    )
    ...
    def open_product_web_report(self):
        ...
        if self.date:
            ...
            if self.location_ids:
                prod_ctx = self.env["product.product"].with_context(
                    to_date=self.date, location=self.location_ids._ids
                )
            else:
                prod_ctx = self.env["product.product"].with_context(
                    to_date=self.date
        ...
        while offset <= limit:
            ...
            product_dict = prod_ctx.search_read(
                domain, fields, offset=offset, limit=inc
            )
