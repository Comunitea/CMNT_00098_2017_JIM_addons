...
class jsProductPricelist(http.Controller):
    ...
    def find_duplicates(self, tmode=0):
        ...
        if request.env.user.has_group('base.group_system'):
            ...
            test_mode = int(tmode) # Test mode como entero (0-1)
            debug_processed = list() # Creamos una lista para guardar los valores
            pricelists_names = set() # Un set para guardar las listas de precios
            pricelist_item_model = request.env['product.pricelist.item'].sudo()
            prices_precision = request.env['decimal.precision'].precision_get('Product Price')
            request.env.cr.execute("SELECT count(id), company_id, pricelist_id, base, applied_on, product_tmpl_id, product_id, date_start, date_end, min_quantity, fixed_price, price_discount, price_min_margin, price_max_margin, base_pricelist_id, compute_price, percent_price, categ_id, price_surcharge, price_round, tiered_price \
                ...
                if repeated > 1:
                    ...
                    if len(rules_to_delete) == repeated_to_delete_count:
                        ...
                        for rule in rules_to_delete:
                            ...
                            # Sacamos la información de los productos
                            debug_processed.append({
                                'id': '%s [%s]' % (rule.id, rule.pricelist_id.name),
