...
class StockWarehouseSGA(models.Model):
    ...
    sga_integrated = fields.Boolean(
        "Integrado con Mecalux",
...
# ~ sga_catch_weight = fields.Selection(
# ~ [("1", "True"), ("0", "False")],
# ~ "Catch weight",
# ~ help="1 Indica si se captura el peso del producto\n"
# ~ "0 (por defecto) se coge el peso de la ficha de producto",
# ~ default="0",
# ~ )
# ~ sga_sustitute_qty = fields.Float(
# ~ "Sustitute qty",
# ~ help="Si se informa sustitute_prod_code, es la cantidad\n"
# ~ "de producto a sustituir",
# ~ default=0,
# ~ )
# ~ sga_disable_alt_product = fields.Selection(
# ~ [("1", "True"), ("0", "False")],
# ~ "Disable alt product",
# ~ help="1 Se deshabilita el producto alternativo\n"
# ~ "0 (por defecto) No se deshabilita",
# ~ default="1",
# ~ )
...
class StockPickingSGA(models.Model):
    ...
    def _get_action_done_bool(self):
        return (
            True
            if self.env["ir.config_parameter"].get_param("picking_auto")
            == u"True"
            else False
        )

    sga_operation = fields.Selection(
        [
            ("A", "Alta"),
            ("M", "Modificacion"),
            ("B", "Baja"),
    ...
    stock_picking_sgatype_id = fields.Many2one(
        "stock.picking.sgatype", string="Tipo de albaran/venta (SGA)"
    )
    stock_picking_sgatype_code = fields.Char(
        related="stock_picking_sgatype_id.code"
    )
    stock_picking_sgatype_description = fields.Char(
        related="stock_picking_sgatype_id.description"
    )

    sga_priority = fields.Integer("Priority", default=100)

    delivery_inst = fields.Char("Avisos para la entrega (SGA)", size=255)
    verify_stock = fields.Selection(
        [("1", "True"), ("0", "False")], "Verifica stock", default="0"
    )
    ...
    shipping_city = fields.Char(
        compute=compute_sga_shipping_info, string="Provincia de entrega"
    )
    shipping_partner_name = fields.Char(
        compute=compute_sga_shipping_info, string="Nombre cliente"
    )
    account_code = fields.Char(
        compute=compute_sga_shipping_info, string="Codigo cliente"
    )
    action_done_bool = fields.Boolean(
        "Validación automática", default=_get_action_done_bool
    )
    do_backorder = fields.Selection(
        [("default", "Por defecto"), ("yes", "Si"), ("no", "No")],
        "Crea entrega parcial",
        default="default",
    )
    ...
    def check_write_in_pm(self, vals):
        ...
        fields_list = sorted(
            list(set(vals).intersection(set(fields_to_check)))
        )
    ...
    def write(self, vals):
        if "action_done_bool" in vals:
            for pick in self:
                pick.message_post(
                    body="El albaran <em>%s</em> <b>ha cambiado el estado de validacion automatica a</b> <em>%s</em>"
                    % (pick.name, vals["action_done_bool"])
                )
        if self.check_write_in_pm(vals):
            raise ValidationError(
                "No puedes modificar operaciones si esta enviado a Mecalux"
            )
    ...
    def move_to_NE(self):
        ...
        picks = self.filtered(
            lambda x: x.sga_integrated and x.sga_state in sga_states_to_NE
        )
    ...
    def new_mecalux_file(self, operation=False, force=False):
        ...
        picks = []
        pick_to_check = self.filtered(
            lambda x: x.state in states_to_check and not force
        )
        if pick_to_check and pick_to_check[0]:
            view = self.env.ref("sga_file.stock_mecalux_confirm_wizard")
            wiz = self.env["stock.mecalux.confirm"].create(
                {"pick_id": pick_to_check.id}
            )
            return {
        ...
        for pick in self.filtered(
            lambda x: x.state in states_to_send or force
        ):
            if not pick.partner_id:
                raise UserError(
                    "No puedes enviar un albaran sin asociarlo a una empresa"
                )
            ...
            new_sga_file = (
                ...
                .check_sga_file(
                    "stock.picking",
                    pick.id,
                    pick.picking_type_id.sgavar_file_id.code,
                )
            )
            if new_sga_file:
    ...
    def do_pick(self, sga_ops_exists, bool_error=True):
        ...
        _logger.info(
            "Entro en do_pick para el albaran %s\n Con valores: sga_ops_exists = %s"
            % (self.name, sga_ops_exists)
        )
        ...
        if not sga_ops_exists:
            ...
            self.message_post(
                body="Pick <em>%s</em> ha sido realizado en Mecalux pero los <b>ids no se encuentran en ODOO</b>."
                % (self.name)
            )
        ...
        pick_ops_product = self.pack_operation_product_ids.filtered(
        ...
        if action_done_bool:
            all_done = True
            do_transfer = True
            op_not_done = self.pack_operation_product_ids.filtered(
            ...
            if op_not_done:
                ...
                display_name_ids = [
                    "%s (Cant: %s)"
                    % (x.product_id.display_name, (x.product_qty - x.qty_done))
                    for x in op_not_done
                ]
            if all_done:
                ...
                self.message_post(
                    body="El albaran <em>%s</em> <b>ha sido validado por Mecalux</b>. Todas las operaciones OK"
                    % (self.name)
                )
                sga_state = "MT"
                ...
                _logger.info(
                    "all_done: {} con do_backorder {} y do_transfer".format(
                        all_done, self.do_backorder, do_transfer
                    )
                )
                ...
                if do_transfer:
                    if all_zero:
                        _logger.info(
                            "Albaran sin nada hecho %s No se valida"
                            % self.name
                        )
                        ...
                        new_name = (
                            ...
                            .browse(self.picking_type_id.id)
                        ...
                        self.message_post(
                            body=(
                                "La orden <em>%s</em> la orden ha sido cerrada en Mecalux sin realizar nada</br>Nuevo nombre en Odoo<b>%s</b>"
                            )
                            % (old_name, new_name)
                        )
                        return bool_error
                        ...
                        _logger.info(
                            "Valido con do_new_transfer {}".format(
                                wiz.display_name
                            )
                        )
                        sga_state = "MT"
                        ...
                        _logger.info(
                            "Albaran %s validado %s" % (self.name, partial)
                        )
                        self.message_post(
                            body="Pick <em>%s</em> <b>ha sido validado en Mecalux </b> %s %s"
                            % (self.name, partial, display_name_ids)
                        )
        self.sga_state = sga_state

        _logger.info(
            "Propago los valores de Mecalux a los elabaranes enlazados "
        )
        ...
        for line in sga_file_lines:
            ...
            if len(line) == LEN_HEADER:
                ...
                if pick:
                    ...
                    pick.date_done = sga_file_obj.format_from_mecalux_date(
                        date_done
                    )
                    if sga_state == "CANCEL":
                        _logger.info(
                            ">> Albaran: %s CANCELADO EN MECALUX" % pick.name
                        )
                        pick.sga_state = "MC"
                        pick.message_post(
                            body="Pick <em>%s</em> <b>ha sido cancelado en Mecalux</b>."
                            % (pick.name)
                        )
                        pick = False
                        continue
                    ...
                    bool_error = False
                    str_error = (
                        "Codigo de albaran %s no encontrado o estado incorrecto en linea ...%s "
                        % (rec_order_code, n_line)
                    )

                    error_message = (
                        u"Albaran %s no encontrado o en estado incorrecto."
                        % (rec_order_code)
                    )
                    self.create_sga_file_error(
                        sga_file_obj,
                        n_line,
                        "CRP",
                        pick,
                ...
                st = 186
                en = st + 12
                qty_done = sga_file_obj.format_from_mecalux_number(
                    line[st:en].strip() or 0, (12, 7, 5)
                )

                st = 210
                en = st + 10
                op_id = sga_file_obj.format_from_mecalux_number(
                    line[st:en].strip() or 0, (10, 10, 0)
                )
                op = self.env["stock.pack.operation"].search(
                    [("id", "=", op_id)]
                )

                if op_id and not op:
                    bool_error = False
                    error_message = u"Op %s no encontrada en el albaran %s" % (
                        op_id,
                        pick.name,
                    )
                    self.create_sga_file_error(
                ...
                if op:
                    _logger.info(
                        ">> Linea % s del albaran: %s. Producto: %s Cantidad hecha %s"
                        % (
                            op.id,
                            pick.name,
        ...
        for line in sga_file_lines:
            ...
            if abs(len(line) - LEN_HEADER) < 2:
                ...
                if pick.sga_state != "PM":
                    pick.message_post(
                        body="Pick <em>%s</em> <b>ha sido realizado en Odoo antes que en Mecalux</b>."
                        % (pick.name)
                    )
                ...
                st = 378
                en = st + 10
                pick_weight = sga_file_obj.format_from_mecalux_number(
                    line[st:en].strip() or 0, (10, 10, 0)
                )

                st = 388
                en = st + 10
                pick_packages = sga_file_obj.format_from_mecalux_number(
                    line[st:en].strip() or 0, (10, 10, 0)
                )
                ...
                st = 90
                en = st + 14
                date_done = sga_file_obj.format_from_mecalux_date(
                    line[st:en].strip()
                )
                vals = {
                    "sga_state": "MT",
