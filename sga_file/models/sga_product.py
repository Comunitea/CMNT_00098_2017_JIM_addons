...
class SGAProductCategory(models.Model):
        ...
        if values.get("code", False):
            if self.product_count:
                raise ValidationError(
                    "No puedes cambiar el código a una categoría con articulos asociados"
                )

        fields_to_check = ("name", "code")
        fields_list = sorted(
            list(set(values).intersection(set(fields_to_check)))
        )
        if fields_list:
            values["sga_state"] = "PA"
    ...
    def create(self, values):
        ...
        if not values.get("code", False):
            new_code = (
                self.env["ir.sequence"].sudo().next_by_code("product.category")
            )
    ...
    def export_category_to_mecalux(self, operation=False):
        ...
        if not ids:
            raise ValidationError(
                "No hay registros modificados para actualizar"
            )
        if operation:
...
class SGAProductPackaging(models.Model):
    ...
    sga_min_quantity = fields.Float(
        "SGA min quantity", default=1, help="Cantidad minima a servir"
    )
    sga_operation = fields.Char("SGA Operation", default="A")
    sga_containertype_code_id = fields.Many2one(
        "sga.containertype",
        "Tipo de contenedor",
        help="Tipo de contenedor de Mecalux",
    )
    sga_containertype_code = fields.Char(
        related="sga_containertype_code_id.code"
    )
    sga_desc_containertype_code = fields.Char(
        related="sga_containertype_code_id.description"
    )
    ...
    def default_get(self, fields):
        ...
        if (
            self._context.get("template_id")
            and "model_id" in fields
    ...
    def create(self, vals):
        ...
        if (
            new_sga.product_tmpl_id
            and new_sga.product_tmpl_id.sga_state != "AC"
        ):
...
class SGAProductProduct(models.Model):
    ...
    def _get_sga_names(self):
        ...
        ctx.update({u"display_default_code": False, u"lang": u"es_ES"})
        for product in self:
    ...
    sga_prod_shortdesc = fields.Char(
        "Nombre Radiofrecuencia", compute="_get_sga_names"
    )
    sga_stock = fields.Float("Stock (SGA)", help="Last PST from Mecalux")
    sga_change_material_abc = fields.Selection(
        [("0", "NO"), ("1", "SI")],
        default="1",
        string="Cambio rotabilidad",
        required=True,
    )
    sga_material_abc_code = fields.Selection(
        [("A", "A"), ("B", "B"), ("C", "C")],
        default="C",
    ...
    def write(self, values):
        ...
        if not fields_list:
            return res
        icp = self.env["ir.config_parameter"]
        create_product_product = self._context.get(
    ...
    def new_mecalux_file(self, operation=False):
        ...
        try:
            ...
            new_sga_file = (
                self.env["sga.file"]
                .with_context(ctx)
                .check_sga_file("product.product", ids, code="PRO")
            )
            if new_sga_file:
        ...
        for product in self:
            if (
                ...
                product.message_post(
                    body=u"Error en envío a Mecalux <em>%s</em> <b>%s</b>."
                    % (
                        product.display_name,
                        "Comprueba categoria asociada y el tipo de producto",
                    )
                )
            else:
...
class SGAProductTemplate(models.Model):
    ...
    sga_state = fields.Selection(
        ...
        default="PA",
        help="Estado integracion con mecalux",
        compute="_compute_sga_state",
        readonly=True,
        store=False,
    )
    sga_dst = fields.Many2one(
        "sga.destination", "Ubicación-Destino", default=_get_default_dst
    )
        ...
        if vals.get("type", False) in SGA_PRODUCT_TYPES:
            ...
            if create_sga_packaging:
                sga_pack_vals = {
                    ...
                    "qty": 500,
                    "name": "Mecalux",
                    "sga_containertype_code_id": self.env["sga.containertype"]
                    .search([("code", "=", "EU")])
                    .id,
                }
    ...
    def write(self, values):
        ...
        fields_to_check = (
            ...
            "display_name",
            "name",
            "sga_material_abc_code",
            "sga_change_material_abc",
            "packaging_ids",
            "product_variant_ids",
        )
        fields_list = sorted(
