...
class SGAfileerror(models.Model):
        ...
        for line_d in sga_file:
            try:
                ...
                if not error_file:
                    ...
                    self.refresh_sga_state(
                        val["object_type"], val["object_id"], line
                    )
                pool_ids.append(error_file.id)
            except:
                sga_file_obj.write_log(
                    "-- ERROR >> Error Al procesar el fichero:\n%s"
                    "\nComprueba los valores de la linea %s"
                    % (sga_file_obj.sga_file, line_number)
                )
    ...
    def refresh_sga_state(self, object_type, object_name, line):
        if object_type in ("PRE", "SOR"):
            ...
            if object:
                object.message_post(
                    body="Pick <em>%s</em> <b>Error en </b>.\n%s"
                    % (object.name, line)
                )
