...
class StockInventorySGA(models.Model):
    ...
    def import_inventory_STO(self, file_id):
        ...
        for line in sga_file_lines:
            ...
            if company_id.ref not in inventories.keys():
                stock_inv_vals = {
                    'name': u'%s (%s)' %(sga_file_name,company_id.ref),
                    'location_id': location_id.id,
                    'filter': 'partial',
                    'company_id': company_id.id
                }
                domain_inv = [('location_id','=',location_id.id),
                              ('company_id', '=', company_id.id),
                              ('state', '=', 'confirm')]

                inventory = self.env['stock.inventory'].create(stock_inv_vals)
                pool_ids.append(inventory.id)
                inventory.prepare_inventory()
                inventories [company_id.ref] = inventory.id
            ...
            if quantity != qty:
                ...
                new_line_vals = {
                    ...
                    'product_qty': new_qty,
                    'company_id': company_id.id,
                    'inventory_id': inventories[company_id.ref]
                }

                domain_stock_inv = [('product_id', '=', product_id.id),
                                    ('location_id', '=', location_id.id),
                                    ('inventory_id.state', '=', 'confirm')]
                inv_line = self.env['stock.inventory.line'].search(domain_stock_inv)
                if inv_line:
                    inv_line.unlink()
                new_line = self.env['stock.inventory.line'].create(new_line_vals)

            # for stock_inv in stock_inv_pool:
            #     stock_inv.sudo().action_done()
        if not len(inventories.keys()):
            raise ValidationError("No se han creado ningun inventario")
        return pool_ids