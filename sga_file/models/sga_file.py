...
class ConfigPathFiles(models.TransientModel):
    ...
    path_files = fields.Char(
        "Files Path",
        help="Path to SGA Mecalux exchange files. Must content in, out, error, processed and history folders\nAlso a scheduled action is created: Archive SGA files",
    )
    product_auto = fields.Boolean(
        "Auto update productos",
        help="Enviar cambios en productos y clientes automaticamente al servidor",
        default=False,
    )
    picking_auto = fields.Boolean(
        "Auto validación de picks",
        help="Valor por defecto para autovalidacion de Mecalux",
    )
    inventary_auto = fields.Boolean(
        "Auto validación de inventarios",
        help="Validación automatica de ajustes de inventarios creados desde Mecalux",
    )
    ...
    def get_default_path_files(self, fields):
        ...
        res["path_files"] = icp.get_param("path_files")
        return res
    ...
    def get_default_product_auto(self, fields):
        res = {}
        icp = self.env["ir.config_parameter"]
        res["product_auto"] = icp.get_param("product_auto")
        return res
    ...
    def get_default_picking_auto(self, fields):
        res = {}
        icp = self.env["ir.config_parameter"]
        res["picking_auto"] = icp.get_param("picking_auto")
        return res
    ...
    def set_path_files(self):
        ...
        if os.path.isdir(self.path_files):
            try:
                ...
                raise UserError(
                    "Error creating directories in %s" % self.path_files
                )
        else:
        ...
        if self.path_files:
            icp = self.env["ir.config_parameter"]
            icp.set_param("path_files", self.path_files)
...
class MecaluxVars(models.Model):
    ...
    length_int = fields.Integer(
        "L. Entera",
        help="Longitud entera(nº de digitos de la parte entera en un tipo 'float')",
    )
    length_dec = fields.Integer(
        "L. Decimal",
        help="Longitud decimal(nº de digitos de la parte decimal en un tipo 'float')",
    )
    mecalux_type = fields.Selection(
        [
            ("A", "Alfanumerico"),
            ("N", "Numerico"),
            ("B", "Booleano"),
...
class MecaluxFile(models.Model):
    ...
    _sql_constraints = [
        (
            "code_uniq",
            "unique (code, version)",
            "Este codigo/version de archivo ya existe!",
        ),
    ]
...
class MecaluxFileHeader(models.Model):
    ...
    state = fields.Selection(
        [("W", "En espera"), ("P", "Procesado"), ("E", "Error")]
    )
    errors = fields.Integer(
        "Errores",
        help="Numero de veces que debe aparecer el error antes de mover a error",
        default=1,
    )
    file_type = fields.Selection(
        [
            (ODOO_READ_FOLDER, "De Mecalux a Odoo"),
            (ODOO_WRITE_FOLDER, "De Odoo a Mecalux"),
        ],
        string="Tipo de fichero (I/O)",
        translate=True,
    )
    ...
    def set_file_name(self, file_type=False, version=False, file_date=False):
        ...
        if file_type and version and datetime == False:
            ...
            version = int(version)
            filename = u"%s%02d%04d%02d%02d%02d%02d%02d" % (
                file_type,
                version,
                file_date.year,
                file_date.month,
    ...
    def archive_sga_files(self, create=False):
        ...
        def create_month_dir(dir, frommonth=u"201801", move=True):
            ...
            while str_from < (datetime.now() - timedelta(days=62)):
                dir_name = "{}{}{}".format(
                    dir, "%04d" % str_from.year, "%02d" % str_from.month
                )
    ...
    def format_to_mecalux_float(
        self, value, length_in=(12, 7, 5), default=0, fillchar="0"
    ):
    ...
    def format_from_mecalux_number(
        self, value, length_in=(12, 7, 5), default=False, fillchar="0"
    ):
        length, length_int, length_dec = length_in
        value = str(value)
        if length_dec > 0:
            res = (
                value[0:length_int]
                + "."
                + value[length_int : length_int + length_dec]
            )
            return float(res)
        else:
    ...
    def format_from_mecalux_date(self, mec_date, long=True):
        ...
        if long:
            ...
            return datetime(
                int(mec_date[0:4]), int(mec_date[4:6]), int(mec_date[6:8])
            )
    ...
    @api.model
    def create_new_sga_file(
        self, sga_filename, dest_path=ODOO_READ_FOLDER, create=True, version=0
    ):
        ...
        if create:
            ...
            if self.env["sga.file"].search(domain):
                version += 1
                sga_filename = "%s%02d" % (sga_filename[0:17], version)
                return self.create_new_sga_file(
                    sga_filename, dest_path, create, version
                )
    ...
    def create(self, vals):
        ...
        if sga_file:
            ...
            if res:
                ...
                raise ValidationError(
                    _(
                        "Error al acceder/crear el archivo:\n%s."
                        % sga_file.sga_file
                    )
                )
        return sga_file
    ...
    def check_sga_name(self, filename, path):
        ...
        if sga_file:
            sga_file.unlink()
        sga_file = self.create_new_sga_file(
            filename, ODOO_READ_FOLDER, create=False
        )
        if not sga_file:
    ...
    def import_file_from_mecalux(self, file_code=False):
        ...
        try:
            if self.file_code == "CSO":
                ...
                process = self.env["stock.inventory"].import_inventory_STO(
                    self.id
                )
                ...
                process = self.env["sga.file.error"].import_mecalux_EIM(
                    self.id
                )
    ...
    def process_sga_files(self, file_type=False, folder=ODOO_READ_FOLDER):
        ...
        print("Buscando ficheros en >> {}".format(global_path))
        pool_ids = []
    ...
    def move_file(self, folder, file_name=False):
        ...
        try:
            ...
            self.write_log(
                "-- Error al mover >> %s a %s" % (self.name, new_path)
            )

    def create_global_PST(self):
        data_file = datetime.now()
        sga_file_name = self.set_file_name("PST", 3, data_file)
        new_sga_file = self.create_new_sga_file(
            sga_file_name, ODOO_WRITE_FOLDER, create=True
        )
        if new_sga_file:
    ...
    def check_sga_file(
        ...
        def get_line(sgavar, model_pool):
            ...
            for model in model_pool:
                ...
                var_str = ""
                try:
                    new_sga_file.write_log(
                        "    Modelo: {} >> {}".format(model, model.name),
                        header_line=False,
                    )
                except:
                    new_sga_file.write_log(
                        "    Modelo: {}".format(model), header_line=False
                    )
                ...
                for val in sgavar.sga_file_var_ids:
                    ...
                    if self._context.get(val.name, False):
                        var_str = self.odoo_to_mecalux(
                            self._context.get(val.name),
                            length,
                            val.mecalux_type,
                            val.default,
                            val.fillchar,
                    ...
                    # No es Listado de lineas
                        ...
                        if value == "" and not val.default:
                            raise UserError(
                                "Revisa la configuracion del campo %s del modelo %s"
                                % (val.name, val.sga_file_id)
                            )
                        ...
                        new_sgavar = self.env["sgavar.file"].search(
                            [("code", "=", val.default)], limit=1
                        )
                        if new_sgavar.filter:
                            new_model = new_model.filtered(
                                eval(new_sgavar.filter)
                            )
        ...
        if not sgavar:
            return
            raise ValidationError(
                "No se ha encontrado un modelo para ese tipo de fichero %s"
                % code
            )
        ...
        if not ids:
            return
            raise ValidationError(
                "No se ha encontrado ningun registro para procesar"
            )
        model_pool = self.env[model].browse(ids)

        if not model_pool:
            raise ValidationError(
                "No se ha encontrado ningun registro de %s" % model
            )

        data_file = datetime.now()
        sga_file_name = self.set_file_name(
            sgavar.code, sgavar.version, data_file
        )
        if not sga_file_name:
            raise ValidationError("Error en el nombre del fichero")

        new_sga_file = self.create_new_sga_file(
            sga_file_name, ODOO_WRITE_FOLDER, create=create
        )

        if not new_sga_file:
            raise ValidationError(
                "Error. Revisa el fichero del log para mas detalles"
            )
        hora_inicio = datetime.now()
        new_sga_file.write_log(
            "    Fichero {}\n    Hora inicio: {}".format(
                new_sga_file.name, datetime.now()
            ),
            header_line=False,
        )
        ...
        new_sga_file.write_log(
            "    Hora fin: {}. Tiempo empleado: {}".format(
                datetime.now(), datetime.now() - hora_inicio
            ),
            header_line=False,
        )
        return new_sga_file
    ...
    def odoo_to_mecalux(
        self, value, length_in, type, default=False, fillchar=False
    ):
        def type_A(value):
            # Tipo string
        ...
        if type == "A":
            ...
            if value:
                value = self.format_to_mecalux_date(
                    datetime.strptime(
                        value, tools.DEFAULT_SERVER_DATETIME_FORMAT
                    )
                )
            else:
                value = ""
