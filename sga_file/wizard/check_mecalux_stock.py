    ...
    def regularize(self):
        ...
        if self.tag_ids:
            product_ids = product_ids.filtered(
                lambda x: self.tag_ids in x.tag_ids
            )
        ...
        str = (
            "RESUMEN de generacion del fichero: {}"
            "\n -- Productos no incluidos: {}"
            "\n -- Productos incluidos: {}"
            "\n -- Inicio escritura fichero: {}"
            "\n -- Fin escritura fichero: {} Tiempo empleado: {}".format(
                new_sga_file.name,
                len(not_product),
                len(ids),
                hora_inicio,
                datetime.now(),
                datetime.now() - hora_inicio,
            )
        )
