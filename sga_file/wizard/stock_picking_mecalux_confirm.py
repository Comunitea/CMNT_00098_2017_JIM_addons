...
class StockMecaluxConfirm(models.TransientModel):
    ...
    def process_force_and_send(self):
        self.ensure_one()
        self.pick_id.message_post(body=u"Env a Mecalux<em>%s</em> <b>Forzado y envío </b>." % self.pick_id.name)
    ...
    def process_force(self):
        self.ensure_one()
        self.pick_id.message_post(body=u"Envío a Mecalux<em>%s</em> <b>Forzado</b>." % self.pick_id.name)
        self.pick_id.force_assign()
