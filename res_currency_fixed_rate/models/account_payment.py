...
class AccountPayment(models.Model):
        ...
        if self.company_currency_id and self.currency_id:
            self.rate = (
                ...
                ._get_conversion_rate(
                    self.company_currency_id, self.currency_id
                )
            )
    ...
    amount_company_currency = fields.Monetary(
        string="Payment Amount (Company currency)", required=True
    )
    rate = fields.Float(
        "Exchange rate", digits=(16, 6), readonly="1", compute=get_rate
    )
    ...
    def onchange_fixed_rate(self):
        self.amount_company_currency = self.amount / (
            self.fixed_rate or self.rate or 1
        )
    ...
    def create(self, vals):
        ...
        if currency_id and currency != self.env.user.company_id.currency_id:
            rate = (
                ...
                ._get_conversion_rate(
                    self.env.user.company_id.currency_id, currency
                )
            )
            vals["amount_company_currency"] = vals.get("amount", 0) / (
                rate or 1
            )
        ...
        if self.fixed_rate_bool:
            if not self.fixed_rate:
                raise exceptions.ValidationError(
                    _("You need set exchange rate")
                )
            ctx["fixed_rate"] = self.fixed_rate
        return super(
            AccountPayment, self.with_context(ctx)
