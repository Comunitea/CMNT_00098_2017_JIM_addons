...
class ParameterizationField(models.TransientModel):
    ...
    parameterization_fields = fields.Many2many(
        "ir.model.fields",
        "ir_model_field_js_field_wizard_rel",
        "field_wizard_id",
        "ir_model_field_id",
        domain=set_domain,
        required=True,
        ...
        for group in xee.fromstring(str(view_id.arch_base)).findall(
            ...
            if groups_attr_invisible and group_fields:
                for domain_item in groups_attr_invisible:
                    ...
                    if is_param_domain:
                        fields_dict.update({int(domain_item[2]): group_fields})
    ...
    def __template_translations(self, template_id):
        ...
        field_name = "%s,%s" % (
            ...
            constants.PARAM_TEMPLATE_FIELD,
        )
        self._cr.execute(
            """
			...
			AND src LIKE %s 
			GROUP BY lang, value""",
            (template_name, field_name, template_name),
        )
        ...
        if self.action == "DELETE":
            return {
                "warning": {
                    "title": _("Warning"),
                    "message": _(
                        "All parameterization values relations will be deleted!"
                    ),
                }
            }
    ...
    def _send(self, action, id, name=None):
        ...
        packet.data = {"jim_id": id, "name": name}

        if packet.send() and self.action == "DELETE":
            for value in self.env[constants.PARAMETERIZATION_VALUES].search(
                [("fields", "in", id)]
            ):
                value.fields = [(3, id)]
        ...
        for group, fields in self.parameterization_fields_get():
            for field_name in fields:
                if field_name in selected_fields_list:
                    field = self.parameterization_fields.filtered(
                        lambda r: r.name == field_name
                    )
                    field.ensure_one()

                    group_names = self.__template_translations(group)
                    field_names = field.get_field_translations(
                        "field_description"
                    )
                    merged_names = {
                        k: "[%s] %s"
                        % (group_names.get(k, "Generic"), field_names[k])
                        if group
                        else field_names[k]
