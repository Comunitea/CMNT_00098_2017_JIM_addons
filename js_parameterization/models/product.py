...
class ProductTemplate(models.Model):
    ...
    parameterization_percent_filled = fields.Integer(
        string="Parameterization Percent Completed",
        default=0,
        store=True,
        copy=False,
    )
    ...
    def compute_parameterization_percent(self):
        for record in self:
            ...
            parameterization_fields = self.env[
                constants.PRODUCT_PARAMETERIZATION
            ].fields_get(template_fields)
            ...
            for field, attrs in parameterization_fields.items():
                ...
                if (
                    hasattr(product_parameterization, field)
                    and product_parameterization[field]
                ):
    ...
    def parameterization_modal(self):
        ...
        parameterization = (
            self.get_parameterization() or self.create_parameterization()
        )
