...
class ProductCheckBarcodes(models.TransientModel):
    ...
    def _parse_row_vals(self, row, idx):
        ...
        if not row[0]:
            raise UserError(
                _('Missing default_code in row %s ') % str(idx))
        return res
    ...
    def act_barcodes(self, row_vals, idx):
        ...
        if product:
            ...
            sql = "update product_product set barcode='{}' where id ={}".format(row_vals['new_barcode'], product.id)
            self._cr.execute(sql)
        else:
            _logger.info('No se ha encontrado la referencia {} {}'.format(default_code, idx))
            return False
        return product
    ...
    def check_barcodes(self):
        ...
        for nline in range(1, sh.nrows):
            ...
            _logger.info(_('Evaluando ref %s, %s / %s') % (row_vals['default_code'], idx, sh.nrows - 1))
