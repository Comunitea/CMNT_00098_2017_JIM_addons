...
class TranslateImportWzd(models.TransientModel):
    ...
    def get_res_id(self, vals):
        ...
        if not template_id and vals["template_code"]:
            domain = [
                ("template_code", "=", "{}".format(vals["template_code"]))
            ]
    ...
    def import_translate(self):
        ...
        field_values = (
            []
        )  # ['id', 'lang', 'src', 'name', 'type', 'res_id', 'state']
        ...
        for nline in range(2, sh.nrows):
            ...
            if not template_id:
                ...
                err_vals = {
                    "filename": filename,
                    "line_id": idx,
                    "text": "{}".format(row_vals),
                }
                self.env["translate.import.error"].create(err_vals)
                _logger.info(
                    _(
                        "Error en la fila {}: Id: {}, Ref: {} , Nombre: {}".format(
                            idx, int(row_vals["id"]), template_code, name
                        )
                    )
                )
                continue
            ...
            for field in field_values:
                ...
                if row_vals[field]:
                    ...
                    _logger.info(
                        _(
                            "Línea {} de {}: Update {}. Campo {}".format(
                                nline, sh.nrows, template_id.name, field
                            )
                        )
                    )
    ...
    def action_view_errors(self, fichero):
        self.ensure_one()
        action = self.env.ref(
            "import_translate_custom.translate_import_error_tree_action"
        ).read()[0]
