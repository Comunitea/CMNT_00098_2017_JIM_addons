...
class StockPicking(models.Model):
    ...
    def get_returned_picking(self):
        for pick in self:
            move = pick.move_lines.filtered(lambda x:x.origin_returned_move_id != False)
            pick.returned_picking = move and move[0].origin_returned_move_id.picking_id or False
