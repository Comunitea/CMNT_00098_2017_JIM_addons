    ...
    def get_qty_done(self):
        for line in self:
            moves = self.picking_ids.mapped("move_lines").filtered(
                ...
                and (
                    x.location_id in ("customer", "supplier")
                    or x.location_dest_id in ("customer", "supplier")
                )
            )

    move_int_id = fields.Many2one(
        "stock.move",
        string="Move from RMA",
        help="The move line related" " to the stocked/scraped product",
    )
