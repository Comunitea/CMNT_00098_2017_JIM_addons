...
class CrmClaimRmaMakeBatchRefund(models.TransientModel):
    ...
    claim_line_ids = fields.Many2many("claim.line", string="Claim lines")
    ...
    def default_get(self, fields):
        ...
        if not all(x.stage_id.default_run for x in claim_ids):
            raise exceptions.UserError(
                "No están todas las reclamaciones en estado correcto"
            )
        if claim_ids and claim_ids[0].company_id != self.env.user.company_id:
            raise exceptions.UserError(
                _("Not in %s" % claim_ids[0].sudo().company_id.name)
            )
        ...
        if claim_ids:
            ...
            if not any(
                claim.claim_type.id == claim_type.id for claim in claim_ids
            ):
                raise exceptions.UserError(_("Not all rma have the same type"))
            if not any(
                claim.partner_id.id == partner_id.id for claim in claim_ids
            ):
                raise exceptions.UserError(
                    _("Not all rma have the same partner")
                )
            if not (claim.company.id == company_id.id for claim in claim_ids):
                raise exceptions.UserError(
                    _("Not all rma have the same company")
                )

            picks_domain = [("claim_id", "in", claim_ids.ids)]
            picks = self.env["stock.picking"].search(picks_domain)
            if not all(
                x.state == "done" or x.state == "cancel" for x in picks
            ):
                raise exceptions.UserError(
                    "No están todas las reclamaciones en estado correcto"
                )
    ...
    def make_batch_refund(self):
        ...
        stage = self.claim_ids[0]._stage_find(
            domain=[("default_done", "=", True)]
        )
        ...
        for line in lines_to_invoice:
            invoice_line_vals = {
                ...
                "name": line.name,
                "quantity": self.get_claim_line_qty_to_invoice(line)
                or line.product_returned_quantity,
            ...
            invoice_line_vals = invoice_line._convert_to_write(
                invoice_line._cache
            )
            if line.claim_type.type == "customer":
                account = self.env["account.account"].search(
                    [("code", "=like", "7080%")], limit=1
                )
                invoice_line_vals["account_id"] = account.id
            new_line = invoice_line_obj.create(invoice_line_vals)
            new_line.write(
                {"price_unit": line.unit_sale_price, "name": line.name}
            )
            line.write(
                {
                    "refund_line_id": new_line.id,
                }
            )
        ...
        self.claim_ids.write({"invoice_id": new_invoice.id, "stage_id": stage})
    ...
    def get_claim_line_qty_to_invoice(self, line):
        ...
        if line.claim_id.claim_type.name == "Customer":
            picks = line.claim_id.picking_ids.filtered(
                lambda x: x.claim_id == line.claim_id
                and (
                    x.location_id.usage == "customer"
                    or x.location_dest_id.usage == "customer"
                )
            )
            moves = picks.move_lines.filtered(
                lambda x: x.claim_line_id == line.id
            )
            qty = 0.0
            for move in moves:
                if move.location_dest_id.usage == "customer":
                    ...
                    move.location_id.usage == "customer" and move.to_refund_so
                ):
                    qty -= move.product_qty
            ...
            picks = line.claim_id.picking_ids.filtered(
                ...
                and (
                    ...
                    or x.location_dest_id.usage == "supplier"
                )
            )
            moves = picks.move_lines.filtered(
                lambda x: x.claim_line_id == line.id
            ...
            for move in moves:
                if move.location_dest_id.usage == "supplier":
                    ...
                    move.location_id.usage == "supplier" and move.to_refund_so
                ):
                    qty -= move.product_qty
            line.qty_to_invoice = qty
        return qty
