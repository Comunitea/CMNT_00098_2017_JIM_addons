...
class ClaimMakePicking(models.TransientModel):
    ...
    def _get_common_dest_location_from_line(self, lines):
        ...
        dest_loc = super(
            ClaimMakePicking, self
        )._get_common_dest_location_from_line(lines)
    ...
    def _get_picking_line_data(self, claim, picking, line):
        ...
        res = super(ClaimMakePicking, self)._get_picking_line_data(
            claim, picking, line
        )
        picking_type = self._context.get("picking_type", False)
        if picking_type == "in":
    ...
    def do_picks(self, claim_ids):
        ...
        picks_to_assign = picks.filtered(
        ...
        picks_to_force = picks.filtered(
            lambda x: x.state in ["partially_available", "confirmed"]
            and x.location_dest_id.usage == "internal"
        )
    ...
    def _create_picking(self, claim, picking_type):
        ...
        claim.claim_line_ids.filtered(lambda x: x.state == "draft").write(
            {"state": "confirmed"}
        )
        ...
        claim_ids = []
        res = super(ClaimMakePicking, self.with_context(ctx))._create_picking(
            claim, picking_type
        )
        ...
        self.env["crm.claim"].browse(claim_ids).write(
            {
                "stage_id": claim._stage_find(
                    domain=[("default_run", "=", True)]
                )
            }
        )
