...
class PurchaseOrderLine(models.Model):
    ...
    def create(self, vals):
        ...
        template_sequence = (
            max(line.order_id.mapped("order_line.template_sequence")) + 10
        )
        new_sequence = 0
        cent = 100
        for value in line.product_id.attribute_value_ids:
            ...
            cent = 1
        line.write(
            {"template_sequence": template_sequence, "sequence": new_sequence}
        )
        return line
...
class PurchaseOrder(models.Model):
    ...
    def reorder_sequence_lines(self):
        ...
        while lines:
            ...
            template_lines = lines.filtered(
                lambda x: x.product_id.product_tmpl_id.id == product_template
            )
            template_lines.with_context(ctx).write(
                {"template_sequence": line_0.template_sequence}
            )
