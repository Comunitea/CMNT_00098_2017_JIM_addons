...
class WizDoneCancelPicks(models.TransientModel):
    ...
    picking_type_id = fields.Many2one(related='picking_done_id.picking_type_id')
...
class WizReturnDestMoveCancelled(models.TransientModel):
    ...
    location_id = fields.Many2one('stock.location', string="Location to check", help ="This is the location dest for done picks\n and location origin for cancelled picks")
    location_dest_id = fields.Many2one('stock.location', string="Location dest", help ="This is the location dest for cancelled picks")
    action_done = fields.Boolean('Auto validation', default=False)
    pick_ids = fields.Many2many('wiz.done.cancel.picks', string='Picking done to canceled', copy=False)
    ...
    def get_sql_str(self):
        str = "select sp1.id, sp2.id, count(sm1.id) from stock_move sm1 " \
        ...
        str = str + "and sm1.location_dest_id=%s " % self.location_id.id if self.location_id else str
        str = str + "and sm2.location_dest_id=%s " % self.location_dest_id.id if self.location_dest_id else str
        str = str + "and sp1.picking_type_id=%s " % self.picking_type_id.id if self.picking_type_id else str
        str = str + "and sp2.picking_type_id=%s " % self.dst_picking_type_id.id if self.dst_picking_type_id else str
