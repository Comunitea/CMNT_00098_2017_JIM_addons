...
class WizRegularizeLocation(models.TransientModel):
    ...
    product_id = fields.Many2one(
        "product.product", string="Product to regularize"
    )
    ...
    def regularize(self):
        ...
        inventory = self.get_inventory_for(
            self.product_id, self.location_id, self.env.user.company_id
        )
        flds = ["product_id", "qty"]
    ...
    def regularize_bis(self):
        ...
        quants = self.env["stock.quant"].search(
            domain, order="id desc, qty asc", limit=limit
        )
        ...
        for quant in quants.filtered(lambda x: x.qty > 0):
            ...
            move_vals = self.env["stock.move"].new(
                self.get_mov_vals(
                    ...
                    quant.product_id.id,
                    quant.qty,
                    pos_pick.location_id,
                    pos_pick.location_dest_id,
                )
            )
            move_vals.onchange_product_id()
            move_vals.product_uom_qty = quant.qty
    ...
    def get_mov_vals(
        self, pick, product_id, qty, location_id, location_dest_id
    ):
        vals = {
            "product_id": product_id,
            "product_uom_qty": qty,
    ...
    def regularize_quants(self):
        ...
        for quant in quants:
            cont -= 1
            if quant["qty"] == 0:
                print("{} Trato {}".format(cont, quant["product_id"][1]))
                ...
                for quant_to_check in quant_to_check_ids:
                    print("Quant nº {}".format(quant_to_check.id))
                    res = quant_to_check.sudo()._quant_reconcile_negative(
                        False
                    )

        domain_neg = [("qty", "<", 0)] + domain
        quants = self.env["stock.quant"].read_group(
            domain_neg, flds, "product_id"
        )
        cont = len(quants)
        for quant in quants:
            cont -= 1
            ...
            domain_pos = [
                ("qty", ">", 0),
                ("product_id", "=", quant["product_id"][1]),
            ] + domain
            quant_to_check_ids = self.env["stock.quant"].search(
                domain_pos, order="qty desc"
            )
