...
class AccountInvoiceReport(models.Model):
    ...
    commercial_partner_ref = fields.Char(string='Empresa asociada. Ref', readonly=True)
    ...
    def _sub_select(self):
        ...
        res = res + """,  partner.ref || ' - ' || partner.name as commercial_partner_ref, ai_partner.ref || ' - ' || ai_partner.name as ref """
        return res

    def _from(self):
        res = super(AccountInvoiceReport, self)._from()
        res = res + """ join res_partner ai_partner on ai.partner_id = ai_partner.id """
        return res
    def _group_by(self):
        res = super(AccountInvoiceReport, self)._group_by()
        res = res + """, partner.ref, partner.name, ai_partner.name, ai_partner.ref """
        return res
