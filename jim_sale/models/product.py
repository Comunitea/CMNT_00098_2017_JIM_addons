...
class ProductTemplate(models.Model):
    ...
    customer_tmpl_prices = fields.One2many(
        "customer.price", "product_tmpl_id", "Customer Prices"
    )
    customer_prices_count = fields.Integer(
        compute="_get_customer_prices_count", string="#Prices"
    )
...
class ProductProduct(models.Model):
    ...
    customer_product_prices = fields.One2many(
        "customer.price", "product_id", "Customer Prices"
    )
    product_item_ids = fields.One2many(
        "product.pricelist.item", "product_id", "Pricelist Product Items"
    )
    customer_prices_count = fields.Integer(
        compute="_get_customer_prices_count", string="#Prices"
    )
