...
class CustomerPrice(models.Model):
    ...
    date_start = fields.Date(
        "Start Date", index=1, help="Start date for this customer price"
    )
    date_end = fields.Date(
        "End Date", index=1, help="End date for this customer price"
    )
    ...
    def get_customer_price(self, partner_id, product, qty, date=False):
        ...
        customer_prices = self.env["customer.price"].search(
            domain, limit=1, order="min_qty desc"
        )
