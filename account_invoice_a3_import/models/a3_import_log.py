...
class A3ImportLog(models.Model):
    ...
    def import_invoice(self, line, company, refund=False):
        ...
        if letters and letters[0] in SerialMap:
            tags = self.env["account.invoice.tag"].search(
                [("name", "=", SerialMap[letters[0]])]
            )
            if tags:
        ...
        invoice_vals = {
            ...
            "date_invoice": datetime.strftime(
                datetime.strptime(line[6:14], "%Y%m%d"), "%Y-%m-%d"
            ),
    ...
    def import_invoice_line(self, line, company):
        ...
        if fiscal_position_code == "01":
            ...
            iva_domain.append(("description", "in", intracomunity_valid_codes))
        elif fiscal_position_code in ["05", "06"]:
            iva_domain.append(("description", "in", extracomunity_valid_codes))
        else:
            raise exceptions.UserError(
                _("Fiscal position code %s has not a parser")
        ...
        invoice_line_vals = {
            ...
            .search(
                [("code", "=", line[15:25]), ("company_id", "=", company.id)]
            )[0]
            .id,
                ...
                try:
                    ...
                    for line in f.readlines():
                        ...
                        if line_type == "1":  # factura de venta
                            ...
                            invoice_vals = doc_.import_invoice(
                                line, company, True
                            )
                        elif line_type == "9":  # detalle de factura
                            invoice_line_vals = doc_.import_invoice_line(
                                line, company
                            )
                        elif line_type == "C":
                            ...
                            raise exceptions.UserError(
                                _("Line type %s has not a parser") % line_type
                            )
                        if partner:
    ...
    def import_files(self):
        folder = self.env["ir.config_parameter"].search(
            [("key", "=", "a3.import.path")]
        )
        if not folder:
        ...
        import_files = [
            x for x in os.listdir(importation_folder) if x.endswith(".dat")
        ]
        docs = self.env["a3.import.log"]
        for import_file in import_files:
            doc = self.search(
                [("name", "=", import_file), ("state", "in", ("new", "error"))]
            )
            if not doc:
                doc_vals = {
                    "name": import_file,
