...
class PurchaseForecast(models.Model):
    ...
    category_ids = fields.Many2many(
        "product.category", string="Product Categorys", required=False
    )
    ...
    def _get_products(self):
        ...
        if not product_ids:
            raise exceptions.UserError(
                _(
                    "No products founded. No forecast \
                                          lines created"
                )
            )
    ...
    def create_lines(self):
        ...
        for product in products:
            ...
            if self.seller_id:
                ...
                seller_line = product.seller_ids.filtered(
                    lambda x: x.name == seller
                )
            ...
            harbor = (
                seller_line
                and seller_line.name.harbor_ids
                and seller_line.name.harbor_ids[0]
                and seller_line.name.harbor_ids[0].id
                or False
            )

            seller2_line = product.seller_ids.filtered(
                lambda x: x.name != seller
            )
            seller2_line = seller2_line and seller2_line[0] or False
            seller2_id = (
                seller2_line
                and seller2_line.name
                and seller2_line.name.id
                or False
            )
            seller2_price = seller2_line and seller2_line.price or False
            harbor2 = (
                seller2_line
                and seller2_line.name.harbor_ids
                and seller2_line.name.harbor_ids[0]
                and seller2_line.name.harbor_ids[0].id
                or False
            )
    ...
    def show_lines(self):
        self.ensure_one()
        action = self.env.ref(
            "jim_purchase_forecast.action_forecast_lines"
        ).read()[0]
...
class PurchaseForecastLine(models.Model):
    ...
    forecast_id = fields.Many2one(
        "purchase.forecast", "Forecast", readonly=True
    )
    ...
    incoming_months = fields.Float(
        "Incoming Pending (Stock Months)", readonly=True
    )
    incoming_remaining = fields.Float(
        "Incoming Pending (Remaining)", readonly=True
    )
    related_picking_id = fields.Many2one(
        "stock.picking", "Related Pickig", readonly=True
    )
    pending_purchase = fields.Float("Pending Purchases", readonly=True)
    related_purchase_id = fields.Many2one(
        "purchase.order", "Related " "Purchase", readonly=True
    )
