...
class ContainerPickingWizard(models.TransientModel):
    ...
    shipping_container_id = fields.Many2one('shipping.container', string="Container")
    line_ids = fields.One2many('container.picking.line.wizard', 'container_picking_wzd_id')
    volume = fields.Float("Volume")
    available_volume = fields.Float("Available volume (m3)", compute="_available")
    ...
    def add_to_container(self):
        ...
        for line_wz in self.line_ids.filtered(lambda x:x.selected):
            line_wz.picking_id.shipping_container_id = self.shipping_container_id
            line_wz.picking_id.min_date = self.shipping_container_id.date_expected

        for line_wz in self.line_ids.filtered(lambda  x: not x.selected and x.container_id == self.shipping_container_id):
    ...
    def default_get(self, fields):
        ...
        if not shipping_container_id:
            return res
        container = self.env['shipping.container'].browse(shipping_container_id)
        ...
        res['weight'] = 0.00
        domain = ['|', ('harbor_id', '=', False), ('harbor_id', '=', container.harbor_id.id),
                  ('state', '=', 'assigned'),
        ...
        for pick in picking_ids:
            if pick.shipping_container_id == container:
                ...
                selected = False
            lines.append({'picking_id': pick.id,
                          'selected': selected,
