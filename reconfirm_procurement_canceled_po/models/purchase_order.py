        ...
        if not 'force_procurement' in self._context.keys():
            ...
            for purchase in self:
                ...
                for line in purchase.order_line:
                    canceled_proc = line.procurement_ids.filtered(lambda p: p.state == 'cancel' and p.rule_id.id == 141)
                if canceled_proc:
                    purchase_ids += purchase
                    canceled_procurement_ids += canceled_proc
            if purchase_ids:
                view = self.env.ref('reconfirm_procurement_canceled_po.wzd_confirm_procurement')
                wiz = self.env['wzd.confirm.procurement'].create({'purchase_ids': [(4,purchase_ids.ids, 0)], 'procurement_ids': [(4,canceled_procurement_ids.ids, 0)]})
                return {
