...
class ProductProductReportingXlsParser(AccountBalanceReportingPrint):
    def __init__(self, name):
        super(ProductProductReportingXlsParser,
              self).__init__(name)
        product_product_obj = self.env['product.product']
        self.context = self._context
        wanted_list = product_product_obj._report_xls_fields()
        template_changes = product_product_obj()
...
try:
    ...
    from odoo.addons.report_xls.utils import _render

    class ProductProductWebReportingXls(report_xls):

        def __init__(self, name, table,
                     rml=False, parser=False, header=True, store=False):
            super(ProductProductWebReportingXls,
        ...
        def _prepare_col_spec_lines_template(self):
            ...
            return {
                'display_name': {
                    'header': [1, 80, 'text', _('Name')],
                    'lines': [1, 0, 'text', _render("l['display_name']")],
                    'totals': [1, 0, 'text', None],
                },
                'default_code': {
                    'header': [1, 10, 'text', _('Code')],
                    'lines': [1, 0, 'text', _render("l['default_code']")],
                    'totals': [1, 0, 'text', None],
                },
                'tag_names': {
                    'header': [1, 15, 'text', _('Tag names')],
                    'lines': [1, 0, 'text', _render("l['tag_names']")],
                    'totals': [1, 0, 'text', None],
                },
                'web': {
                    'header': [1, 15, 'text', _('Web'),
                               None, self.rh_cell_style_right],
                    'lines': [1, 0, 'number', _render("l['web']"),
                              None, self.aml_cell_style_decimal],
                    'totals': [1, 0, 'text', None]
                },
                'global_real_stock': {
                    'header': [1, 15, 'text', _('Global Stock'),
                               None, self.rh_cell_style_right],
                    'lines': [1, 0, 'number', _render("l['global_real_stock']"),
                              None, self.aml_cell_style_decimal],
                    'totals': [1, 0, 'text', None]
                },
            }
        ...
        def generate_xls_report(self, _p, _xs, data, objects, wb):
            ...
            report_name = "%s (%s) - %s" % (
                report_name, template_name, calc_date)
    ...
    ProductProductWebReportingXls(
        'report.reporting.xls', 'product.product',
        parser=ProductProductReportingXlsParser)
