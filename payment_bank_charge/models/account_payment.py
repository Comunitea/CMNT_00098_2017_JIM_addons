...
class AccountPayment(models.Model):
    ...
    bank_charge_account_id = fields.Many2one(
        "account.account", "Bank charge account", readonly=True
    )
        ...
        if self.bank_charge_bool:
            if not self.journal_id.update_posted:
                raise ValidationError(
                    _(
                        "You cannot add bank charges in this journal."
        ...
        if self.bank_charge_bool:
            ...
            if self.currency_id != loc_currency_id:
                rate = self.currency_id._get_conversion_rate(
                    self.currency_id, loc_currency_id
                )
