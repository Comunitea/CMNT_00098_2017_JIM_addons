...
class SaleOrderLineTemplate(models.Model):
    ...
    order_lines = fields.One2many(
        "sale.order.line", "template_line", copy=True
    )
    ...
    global_available_stock = fields.Float(
        "Stock", related="product_template.global_available_stock"
    )
    ...
    def create(self, vals):
        ...
        if not self._context.get("no_create_line", False):
            ...
            line_vals = vals.copy()
            template_product = self.env["product.template"].browse(
                vals["product_template"]
            )
            ...
            new_line = (
                ...
                .with_context(no_create_template_line=True)
                .create(line_vals)
            )
        ...
        return super(
            ...
            self.with_context(no_create_template_line=True),
        ).create(vals)
    ...
    def create_mal(self, vals):
        ...
        if not self._context.get("no_create_line", False):
            ...
            if orig:
                ...
                new_line_ids |= new_line_ids.with_context(ctx).create(
                    order_line_vals
                )
...
class SaleOrderLine(models.Model):
    ...
    def _get_global_stock(self):
        for line in self:
            if line.product_id:
                line.global_available_stock = line.product_id.web_global_stock
            else:
    ...
    global_available_stock = fields.Float(
        "Stock", readonly=True, compute="_get_global_stock", store=True
    )
    ...
    @api.depends(
        "template_line.lines_qty",
        "product_id.product_tmpl_id.product_attribute_count",
    )
    ...
    def create(self, vals):
        ...
        if not vals.get("template_line", False) and not self._context.get(
            "no_create_template_line", False
        ):
            ...
            new_template = (
                self.env["sale.order.line.template"]
                .with_context(
                    no_create_template_line=True, no_create_line=True
                )
                .create(vals)
            )
    ...
    def _onchange_product_id_check_availability(self):
        ...
        if (
            ...
            warning_mess = {
                ...
                "message": _(
                    "You plan to sell %s %s but you only have %s %s "
...
class SaleOrder(models.Model):
    ...
    template_lines = fields.One2many(
        "sale.order.line.template", "order_id", copy=True
    )
    ...
    def copy(self, default={}):
        return super(
            SaleOrder,
            self.with_context(
                no_create_line=True, no_create_template_line=True
            ),
        ).copy(default)
