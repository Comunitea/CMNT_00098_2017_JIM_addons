...
class SaleManageVariant(models.TransientModel):
    ...
    def button_transfer_to_order(self):
        ...
        super(
            SaleManageVariant,
            self.with_context(
                template_line=template.id,
                active_model="sale.order",
                active_id=template.order_id.id,
            ),
        ).button_transfer_to_order()
    ...
    def _onchange_product_tmpl_id(self):
        ...
        if context["active_model"] == "sale.order.line":
            ...
            if sale_order.template_lines:
                if self.product_tmpl_id.id in [
                    x.product_template.id for x in sale_order.template_lines
                ]:
                    self.product_tmpl_id = False
                    raise ValidationError(
                        _(
                            "You must modify this template from template lines in sale order."
                        )
                    )
