...
class Subprocess(object):
    ...
    def background_run(self, method, record, mode):
        ...
        _logger.info(
            "Ejecutando subproceso para [%s,%i]" % (record._name, record.id)
        )
...
class JSync(object):
    ...
    def __init__(self, odoo_env, settings=None, retries=3):
        ...
        self.settings = settings or self.env[
            "b2b.settings"
        ].get_default_params(
            fields=["url", "conexion_error", "response_error"]
        )
    ...
    def update_b2b_export_table(self, b2b_export_id=False, res_id=False):
        ...
        updated = False
        with api.Environment.manage():
            with registry.RegistryManager.get(
                self.env.cr.dbname
            ).cursor() as new_cr:
                new_cr.autocommit(True)
                ...
                if not b2b_export_id and self.mode == "create" and res_id:
                    ...
                    updated = (
                        env["b2b.export"]
                        .browse(b2b_export_id)
                        .with_context(b2b_evaluate=False)
                        .write({"name": self.name, "rel_id": self.related})
                    )
        ...
        if self.data and type(self.data) is dict:
            for field, value in self.data.items():
                if ":" in field:
                    ...
                    elif modifier == "changed" and (
                        not vals or not obj_new in vals
                    ):
    ...
    def can_send(self):
        ...
        if (
            self.name
            and self.mode
            and (self.mode == "delete" or len(self.data) > 1)
        ):
            ...
            with api.Environment.manage():
                with registry.RegistryManager.get(
                    ...
                    EXPORT_ID = (
                        env["b2b.export"]
                        .search([("res_id", "=", RES_ID)], limit=1)
                        .id
                    )

                    # Send the record comparing internal table reference?
                    RECORD_SEND = bool(
                        (not EXPORT_ID and self.mode == "create")
                        or (EXPORT_ID and self.mode == "delete")
                        or self.mode == "update"
                    )
                    ...
                    if RECORD_SEND and self.related:
                        ...
                        if (
                            not env[record_model]
                            .browse(int(record_id))
                            .is_notifiable_check()
                        ):
                            _logger.warning(
                                "El registro relaccionado ha invalidado el paquete!"
                            )
                            RECORD_SEND = False
    ...
    def send(self, timeout_sec=30, notify=True, **kwargs):
        ...
        if _RECORD_SEND and self.data:
            ...
            if jsync_post and jsync_post.status_code is 200:
                ...
                _logger.debug(
                    debug_msg.format(
                        jsync_post.text, self.name, self.mode, self.data
                    )
                )
                ...
                if _RES_ID:
                    ...
                    if (
                        notify
                        and "show_b2b_notifications" in self.env.user
                        and self.env.user.show_b2b_notifications
                    ):
                        self.env.user.notify_info(
                            "[B2B] %s <b>%s</b> %s"
                            % (self.mode.capitalize(), self.name, self.id)
                        )
                ...
                if (
                    jsync_post
                    and self.settings["conexion_error"]
                    and self.settings["response_error"]
                ):
                    raise ValidationError(
                        "JSync Server Response Error\n%s"
                        % (jsync_post.text.encode("latin1").capitalize())
                    )
        else:

            _logger.info(
                "El paquete para '%s,%s' no se envió en modo '%s' ya que no cumple las condiciones!"
                % (self.name, self.id, self.mode)
            )
