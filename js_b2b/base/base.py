...
class BaseB2B(models.AbstractModel):
	...
	def get_field_translations(self, field='name'):
		...
		self._cr.execute("SELECT lang, value FROM ir_translation WHERE name=%s AND res_id=%s", (field_name, self.id))
			...
			if self._name in item.get_models() and type(item.code) is unicode:
				...
				if b2b['is_notifiable'](self, mode, vals):
					...
					if type(watched_fields) in (list, tuple) and mode == 'update':
						...
						items_dict.update({ item.name: bool(mode in ('create', 'delete') or vals) })
