...
class BaseB2B(models.AbstractModel):
    ...
    def get_field_translations(self, field="name"):
        ...
        # Search active langs but skip 'es' we use 'es_ES' instead
        configured_langs = self.env["res.lang"].search(
            [
                ("active", "=", True),
                ("translatable", "=", True),
                ("code", "!=", "es"),
        ...
        # Default values
        translations = {
            lang.code.replace("_", "-"): origin[field] or None
            for lang in configured_langs
        }
        # Query to get translations
        self._cr.execute(
            "SELECT lang, value FROM ir_translation WHERE name=%s AND res_id=%s",
            (field_name, self.id),
        )
        # Update translations dict
        translations.update(
            {
                lang_code.replace("_", "-"): field_translation
                for lang_code, field_translation in self._cr.fetchall()
                if lang_code.replace("_", "-") in translations
            }
        )
    ...
    def get_base64_report_pdf(self, model_name):
        ...
        docfile = (
            self.env["report"].with_context(ctx).get_pdf([self.id], model_name)
        )
    ...
    def get_field_multicompany(self, field="name"):
        ...
        for value in (
            ...
            .sudo()
            .search([("name", "=", field), ("res_id", "=", res_id)])
        ):
            result_item = False
    ...
    def on_jsync(self):
        ...
        self.ensure_one()
        res_id = "%s,%s" % (self._name, self.id)
        export_found = self.env["b2b.export"].search(
    ...
    def is_notifiable_check(self, mode="create", vals=None):
        ...
        b2b_config = self.env["b2b.item.out"].search(
            [("active", "=", True), ("model", "like", "%%%s%%" % self._name)]
        )
        ...
        for item in b2b_config:
            ...
            if self._name in item.get_models() and type(item.code) is unicode:
                ...
                if b2b["is_notifiable"](self, mode, vals):
                    ...
                    if (
                        type(watched_fields) in (list, tuple)
                        and mode == "update"
                    ):
                        ...
                        items_dict.update(
                            {
                                item.name: bool(
                                    set(vals).intersection(set(watched_fields))
                                )
                            }
                        )
    ...
    def b2b_record(
        ...
        b2b_config = self.env["b2b.item.out"].search(
            [("name", "in", applicable_configs.keys())]
        )
        ...
        if applicable_configs and not importing_file:
            ...
            for item in b2b_config:
                ...
                if mode == "update" and conf_items_after != None:
                    ...
                    if (
                        item.name in conf_items_before.keys()
                        and item.name not in conf_items_after.keys()
                    ):
                        b2b["crud_mode"] = "delete"
                    # Si antes no era notificable y ahora si o lo creamos
                    elif (
                        item.name not in conf_items_before.keys()
                        and item.name in conf_items_after.keys()
                    ) or not self.on_jsync():
                        b2b["crud_mode"] = "create"

                _logger.info(
                    "Paquete para [%s,%i] en modo [%s]",
                    self._name,
                    self.id,
                    b2b["crud_mode"],
                )
                ...
                if applicable_configs.get(item.name) == True:
                    ...
                    if auto_send:
                        packet.send(notify=user_notify)
                    ...
                    if record_on_jsync and packet.mode == "update":
                        ...
                        _logger.info(
                            "El registro '%s,%s' no es notificable pero se actualizó la tabla de exportación!",
                            self._name,
                            self.id,
                        )
    ...
    def get_metadata(self):
        ...
        for i, v in enumerate(metadata):
            ...
            metadata[i]["b2b_notifiable"] = (
                ", ".join(record_notifiable) if record_notifiable else "false"
            )
            metadata[i]["b2b_record_on_jsync"] = (
                "true" if record_on_jsync else "false"
            )
        return metadata
    ...
    def write(self, vals):
        ...
        for record in self:
            ...
            if super(BaseB2B, record).write(vals) and b2b_evaluate:
                record.b2b_record(
                    "update", vals, conf_items_before=items_to_send
                )
        return True
    ...
    def delete_link(self):
        for record in self:
            link = self.env["b2b.export"].search(
                [("res_id", "=", "%s,%s" % (record._name, record.id))], limit=1
            )
            link.unlink()
