...
class B2bController(http.Controller):
    ...
    def index(self, **kw):
        ...
        return http.request.render(
            "js_b2b.b2b_in_home",
            {
                "b2b_in_active": http.request.env["b2b.item.in"].search(
                    [("active", "=", True)]
                ),
                "b2b_in_inactive": http.request.env["b2b.item.in"].search(
                    [("active", "=", False)]
                ),
            },
        )
    ...
    def message(self, **kw):
        ...
        try:
            ...
            # Process item
            result = http.request.env["b2b.item.in"].must_process(
                item_name, item_data, item_action
            )
            if result == True:
        ...
        if http.request.env.user.has_group("base.group_system"):
            ...
            for product in (
                ...
                if execute and product_published != product.website_published:
                    ...
                    try:
                        product.with_context(
                            b2b_evaluate=False
                        ).website_published = product_published
                    except:
                ...
                for variant in product.product_variant_ids:
                    ...
                    variant_must_be_published = bool(
                        variant.active
                        and variant.default_code
                        and variant.force_web != "no"
                    )
                    ...
                    if not product_published and variant_published:
                        ...
                        and not variant_must_be_published
                    ):
                        ...
                        and variant.active
                        and not variant.attribute_names
                    ):
                    ...
                    if (
                        execute
                        and variant_published != variant.website_published
                    ):
                        # No lo evaluamos para que sea más rápido por lo que si hay cambios deberemos
                        ...
                        try:
                            variant.with_context(
                                b2b_evaluate=False
                            ).website_published = variant_published
                        except:
                            variants_to_publish -= 1

            message = (
                "Para hacer efectivos los cambios lanzar con execute=1"
                if not execute
                else "¡Realizado! Si hay cambios tendrás que hacer una sincronización"
            )
