...
class B2bItemsOut(models.Model):
    ...
    _sql_constraints = [
        (
            "b2b_item_out_unique",
            "unique(name)",
            "Name must be unique into B2B Outgoing Items!",
        )
    ]
    ...
    model = fields.Char(
        ...
        required=True,
        translate=False,
        help="Odoo model names to check separated by , or ;",
    )
        ...
        for var, typ in variables.items():
            ...
            if var_content and type(var_content) not in typ:
                raise UserError(
                    _(
                        "Code Error!\nItem: %s\nVar name: %s\nReceived type: %s\nExpected type: %s"
                        % (self.name, var, type(var_content), typ)
                    )
                )
    ...
    def sync_item(self, user_notify=False):
        ...
        for model in self.get_models():
            ...
            if model == "stock.move":
                ...
                search_query = [
                    ("date_invoice", ">=", docs_min_date),
                    ("commercial_partner_id", "in", client_ids),
                ]
                ...
                search_query = [
                    ("date_order", ">=", docs_min_date),
                    ("partner_id.commercial_partner_id", "in", client_ids),
                ]
            elif model in (
                "product.template",
                "product.product",
            ...
            records_ids = (
                self.env[model]
                .with_context(active_test=only_active)
                .search(search_query, order="id ASC")
                .ids
            )
            ...
            for id in records_ids:
                ...
                record_percent = round(
                    (record_number / total_records) * 100, 1
                )
                ...
                record_on_jsync = self.env["b2b.export"].search(
                ...
                if all_notifiable and not record_on_jsync:
                    ...
                    create_records += 1
                    _logger.info(
                        "@@ CREATE %s (%s) WITH ID#%s | COMPLETED: %s"
                        % (self.name, model, id, record_percent_str)
                    )
                    ...
                    delete_records += 1
                    _logger.info(
                        "@@ DELETE %s (%s) WITH ID#%s | COMPLETED: %s"
                        % (self.name, model, id, record_percent_str)
                    )
                    ...
                    all_notifiable
                    and record_on_jsync
                    and self.sync_updates
                    and record.write_date > record_on_jsync.write_date
                ):

                    update_records += 1
                    _logger.info(
                        "@@ UPDATE %s (%s) WITH ID#%s | COMPLETED: %s"
                        % (self.name, model, id, record_percent_str)
                    )
                    record.b2b_record(
                        "update",
                        conf_items_before={record_on_jsync.name: True},
