...
PARAMS = {
    ...
    "base_url": (
        "b2b.public_base_url",
        "https://jimsports.website/b2b_images/",
    ),
...
class B2BSettings(models.TransientModel):
    ...
    url = fields.Char(
        ...
        required=True,
        translate=False,
        help="Set the server URL (http://ip:port/)",
    )
    conexion_error = fields.Boolean(
        "Conexion errors",
        help="Disturb user with conexion errors and do not execute the action",
    )
    response_error = fields.Boolean(
        "Response errors",
        help="Disturb user with response errors and do not execute the action",
    )
    base_url = fields.Char(
        "Public Base URL",
        required=False,
        translate=False,
        help="Images public base URL",
    )
    server = fields.Char(
        "Public Server",
        required=False,
        translate=False,
        help="Set the server IP or domain",
    )
    user = fields.Char(
        "User",
        required=False,
        translate=False,
        help="Set the FTP server username",
    )
    password = fields.Char(
        "Password",
        required=False,
        translate=False,
        help="Set the TP server password",
    )
    ...
    def get_param(self, param, value=None):
        if param in PARAMS:
            ...
            return safe_eval(
                ...
                .sudo()
                .get_param(key_name, repr(default_val))
            )
        return False
    ...
    def update_param(self, param, value=None):
        if param in PARAMS:
            ...
            self.env["ir.config_parameter"].set_param(
                key_name, repr(value or default_val)
            )
