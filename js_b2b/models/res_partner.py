...
class ResPartner(models.Model):
    ...
    vip_web_access = fields.Many2many(
        "res.company",
        "res_partner_res_company_web_access",
    ...
    def __check_vip_web_access_companies_pricelists(self, vals=dict()):
        if vals.get("vip_web_access", False):
            ...
            for record in self:
                self.env.cr.execute(
                    "SELECT company_id FROM ir_property WHERE name LIKE 'property_product_pricelist' and res_id = 'res.partner,%s'",
                    (record.id,),
                )
                ...
                for company in record.vip_web_access:
                    if company.id not in pricelists_company_ids:
                        raise ValidationError(
                            _(
                                "Unable to set web access for %s\nClient don't have pricelist on this company!"
                            )
                            % company.name
                        )
    ...
    def has_valid_emails(self):
        ...
        # Test email/s
        email_valid = (
            validate(_email_rfc822_validation, self.email)
            if self.email
            else None
        )
    ...
    def _check_email_address(self):
        for record in self:
            if record.email and not record.has_valid_emails():
                raise ValidationError(
                    _("Partner email is not valid, check it!")
                )
