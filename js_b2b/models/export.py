...
class B2BExport(models.Model):
    ...
    _sql_constraints = [
        (
            "res_id_unique",
            "unique(res_id)",
            "Export res_id needs to be unique!",
        )
    ]
    ...
    res_id = fields.Char(
        required=True,
        translate=False,
        help="Record and model [model_name,record_id]",
    )
    rel_id = fields.Char(
        required=False,
        translate=False,
        help="Related to [model_name,record_id]",
    )
    ...
    def b2b_pricelists_prices(
        self,
        test_limit=None,
        templates_filter=None,
        pricelists_filter=None,
        variant=None,
        qty=None,
    ):
        ...
        prices = list()
        # Get decimals number
        prices_precision = self.env["decimal.precision"].precision_get(
            "Product Price"
        )
        ...
        def _search_pricelist_quantities(quantities, pricelist_id):
            unique_quantities = set(
                tuple(qty[1] for qty in quantities if qty[0] == pricelist_id)
            )
        ...
        pfilter = (
            [("id", "in", pricelists_filter)] if pricelists_filter else []
        )
        pricelists = tuple(
            self.env["product.pricelist"]
            .search([("web", "=", True), ("active", "=", True)] + pfilter)
            .mapped(lambda p: (p.id, p.name, p.company_id.id))
        )
        ...
        products_ids = tuple(
            ...
            .search(product_search_params, limit=test_limit)
            .ids
        )
        ...
        try:
            ...
            for product_id in products_ids:
                ...
                for pricelist in pricelists:
                    ...
                    # Pricelist filter
                    all_or_this_pricelist = (
                        not pricelists_filter
                        or pricelists_filter[0] == pricelist[0]
                    )

                    # For each quantity
                    for min_qty in _search_pricelist_quantities(
                        quantities, pricelist[0]
                    ):
                        ...
                        product_in_ctx = product.with_context(
                            {"pricelist": pricelist[0], "quantity": min_qty}
                        )

                        # Get all variant prices
                        variants_prices = tuple(
                            product_in_ctx.product_variant_ids.mapped("price")
                        )
                        ...
                        if all(
                            ...
                            product_filter = filter(
                                lambda x: x["pricelist_id"] == pricelist[0]
                                and x["product_id"] == product_id
                                and x["variant_id"] == None
                                and x["quantity"] == 1
                                and x["price"] == price,
                                prices,
                            )
                            ...
                            if (
                                not bool(list(product_filter))
                                and all_or_this_pricelist
                                and all_or_this_quantity
                            ):
                            ...
                            for v in range(len(variants_prices)):
                                ...
                                # Get variant ID
                                variant_id = (
                                    product_in_ctx.product_variant_ids.ids[v]
                                )

                                # Variant filter
                                all_or_this_variant = (
                                    not variant or variant_id == variant
                                )

                                # Get variant price
                                price = round(
                                    variants_prices[v], prices_precision
                                )
        ...
        if prices:
            mode = (
                "update"
                if (templates_filter or pricelists_filter)
                else "replace"
            )
            self.write_to_log(
                "%s -> %s" % (mode, str(prices)), "pricelist_item", "a+"
            )
            self.send_packet("pricelist_item", prices, mode)

    def b2b_customers_prices(
        self,
        lines_filter=None,
        operation=None,
        templates_filter=None,
        ...
        try:
            # Get all prices
            for price_line in self.env["customer.price"].read_group(
                prices_filter,
                (
                    "company_id",
                    "partner_id",
                ...
                if "price" in price_line and (
                    price_line.get("product_tmpl_id")
                    or price_line.get("product_id")
                ):

                    # Get product ID's
                    template_id = (
                        price_line["product_tmpl_id"][0]
                        if price_line.get("product_tmpl_id")
                        else None
                    )
                    variant_id = (
                        price_line["product_id"][0]
                        if price_line.get("product_id")
                        else None
                    )
                    ...
                    price_found = bool(
                        list(
                            filter(
                                ...
                                == price_line["partner_id"][0]
                                and x["product_id"] == template_id
                                and x["variant_id"] == variant_id
                                and x["quantity"] == line_quantity,
                                prices,
                            )
                        )
                    )
                    ...
                    if not price_found and (
                        operation == "delete" or price_line["price"]
                    ):
                        prices.append(
                            {
                                "company_id": price_line["company_id"][0]
                                if price_line["company_id"]
                                else None,
        ...
        if prices:
            mode = (
                "update" if (lines_filter or templates_filter) else "replace"
            )
            self.write_to_log(
                "%s -> %s" % (mode, str(prices)), "customer_price", "a+"
            )
            self.send_packet("customer_price", prices, mode)

    def b2b_products_stock(
        self,
        test_limit=None,
        from_date=None,
        export_all=None,
        ...
        try:
            stock = self.env["exportxml.object"].compute_product_ids(
                all=export_all, from_time=from_date, limit=test_limit, inc=1000
            )
        except Exception as e:
        ...
        if stock:
            ...
            self.write_to_log(
                "%s -> %s" % (mode, str(stock)), "product_stock", "a+"
            )
    ...
    def b2b_delete_old_supplies(self):
        ...
        try:
            supply_plan_ids = self.search(
                [("res_id", "=like", "stock.move,%")]
            ).mapped(
                lambda record: int(
                    record.res_id.split(",")[1] if record.res_id else 0
                )
            )
            for record in self.env["stock.move"].search(
                [
                    ("id", "in", supply_plan_ids),
    ...
    def unlink(self):
        for record in self:
            ...
            if super(B2BExport, record).unlink():
                self.env[res_model].browse(int(res_id)).with_context(
                    b2b_autolinks=False
                ).b2b_record("delete", conf_items_before={resource_name: True})
                ...
                for related in self.search([("rel_id", "=", resource_id)]):
                    ...
                    if related.unlink():
                        self.env[rel_model].browse(int(rel_id)).with_context(
                            b2b_autolinks=False
                        ).b2b_record(
                            "delete", conf_items_before={related_name: True}
                        )
        return True
