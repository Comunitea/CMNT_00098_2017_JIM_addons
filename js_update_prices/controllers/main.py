...
class jsUpdatePrices(http.Controller):
    @http.route(["/js_update_prices"], type="http", auth="user")
    def index(self, **kw):
        return request.render("js_update_prices.prices_edit_index")

    @http.route(["/js_update_prices/run"], type="http", auth="user")
        ...
        if request.env.user.has_group("base.group_system"):
            ...
            if order != "":
                sales = (
                    ...
                    .sudo()
                    .search([("name", "=", order)], order="id")
                )
            else:
                sales = (
                    request.env["sale.order"]
                    .sudo()
                    .search([("state", "=", "draft")], order="id")
                )
            ...
            for order in sales:
                ...
                for line in order.order_line:
                    ...
                    if line.state == "sale" and (
                        ...
                        or line.invoice_status == "to invoice"
                    ):

                        old_price = line.price_unit
                        price_unit = order.pricelist_id.get_product_price(
                            line.product_id, 1, order.partner_id
                        )
                        price_subtotal = order.pricelist_id.get_product_price(
            ...
            return request.render(
                "js_update_prices.prices_edit_batch",
                {
                    "total": numLines,
