            ...
            for company in company_objs:
                ...
                for field in fields_list:
                    ...
                    if model_value:
                        ...
                        partner2.sudo().with_context(b2b_evaluate=False).write(
                            {field: model_value.id}
                        )
                ...
                for field in company_fields_list:
                    ...
                    if model_value:
                        ...
                        model_obj = (
                            self.env[model_name].sudo().search(domain, limit=1)
                        )
                        if model_obj:
                            # Skip if no propagate active:
                            if company.no_propagate_term and field in [
                                "customer_payment_mode_id",
                                "supplier_payment_mode_id",
                            ]:
                                continue
                            # WRITE in new company
                            partner2.sudo().with_context(
                                b2b_evaluate=False
                            ).write({field: model_obj.id})
