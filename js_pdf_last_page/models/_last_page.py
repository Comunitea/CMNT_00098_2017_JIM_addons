# -*- coding: utf-8 -*-
# ESTO AHORA ESTÁ EN LA BASE

class Report(models.Model):
	_inherit = 'report'

	def _low_get_pdf(self, docids, report_name, html=None, data=None):
		""" 
	...
	def get_pdf(self, docids, report_name, html=None, data=None):
		"""
			Se tuvo que modificar porque no encontramos otra forma de poder incrustar una
			última página para cada documento de un PDF, ya que pueden ser multiples facturas en
			un sólo ducumento y cada una debe llevar la última página, además el documento de debe firmar
			con las mismas incorporadas.

			Si en un futuro se necesitan hacer modificaciones ahora es posible sobreescribir _low_get_pdf
			recuperar los argumentos y modificarlos para pasarlos al método superior

			Las páginas finales se guardan en el nuevo argumento fullpages
		"""
		wkhtmltopdf_args = self._low_get_pdf(docids, report_name, html, data)
